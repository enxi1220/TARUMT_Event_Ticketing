...
/*        background: rgba(50, 50, 50, 0.03);*/
      ...
      .report-title {
		...
		xmlhttp.send();
		var xslData = xmlhttp.responseXML;
                
                 function transformXML(xmlData, xslData) {
                    var processor = new XSLTProcessor();
                    processor.importStylesheet(xslData);
                    var htmlData = processor.transformToDocument(xmlData);
                }
                
                 function printReport() {
            var htmlData = new XMLHttpRequest();
            htmlData.open('GET', 'data.xml', false);
            htmlData.send();
            var xslData = new XMLHttpRequest();
            xslData.open('GET', 'report.xsl', false);
            xslData.send();
            var processor = new XSLTProcessor();
            processor.importStylesheet(xslData.responseXML);
            var resultDocument = processor.transformToDocument(htmlData.responseXML);
